...
通过上面的命令可查看当前运行的所有进程。需要注意的是，该命令同时列出了 PID（进程ID）。参数 a 和 u 列出了所有进程的详细信息。
...
结果（因为暂停了 30 秒，所以在这个时间内可以验证一下子进程是否为僵尸进程。）：
...
运行结果中输入 ctrl+c 时就可以看到 `CTRL+C pressed` 的字符串。
...
下面是基于多进程实现的并发的回声服务器的服务端，可以结合第四章的 [echo_client.c](../../ch04/c/echo_client.c) 回声客户端来运行。
