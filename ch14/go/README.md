...
- Sender : 向 AAA 组广播（Broadcasting）信息；
- Receiver : 接收传递到 AAA 组的新闻信息；

下面是两个代码：

- [news_sender.go](./news_sender.go)
- [news_receiver.go](./news_receiver.go)

编译运行：

```
go run news_receiver.go
go run news_sender.go
```
...
因为发送方不停地发送信息，所以无论先运行发送方还是接收方都能接收到信息。
...
- [news_sender_brd.go](./news_sender_brd.go)
- [news_receiver_brd.go](./news_receiver_brd.go)

编译运行：

```
go run news_receiver_brd.go
go run news_sender_brd.go
```
